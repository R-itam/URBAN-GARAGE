...
<body>
   ...
   <br><br>
    ...
    <div class="container-fluid page-header">
        <h1 class="display-3 text-uppercase text-white mb-3">SUV Section</h1>
        <div class="d-inline-flex text-white">
            <h6 class="text-uppercase text-body m-0">Edit SUV</h6>
        </div>
    </div>
    <!-- Page Header Start -->
     ...
     <div class="container-fluid py-5">
        <div class="container pt-5 pb-3">
            ...
            <h1 class="display-4 text-uppercase text-center mb-5">Edit SUV Details</h1>
            <div class="row">
                <div class="col-lg-12 mb-2">
                    <div class="contact-form bg-light mb-4" style="padding: 30px;">
                    <div class="container-fluid py-5">
                        @if (isset($su_edit))
                        @foreach ($su_edit->all() as $all )
                        <form action="{{url('/suveditformaction')}}" method="post" enctype="multipart/form-data">
                            @csrf
                            <input type="hidden" name="suv" value="{{$all->User_id}}">
                            <div class="form-group">
                                <label for="Name">Car Name:</label>
                                <input type="text" name="su_name" class="form-control" value="{{$all->Name}}" required>
                            </div>
                            <div class="form-group">
                                <label for="Name">Car Model:</label>
                                <input type="text" name="su_model" class="form-control" value="{{$all->Car_id}}" required>
                            </div>
                            <div class="form-group">
                                <label for="Name">Car Details:</label>
                                <input type="text" name="su_detail" class="form-control" value="{{$all->Details}}" required>
                            </div>
                            <div class="form-group">
                                <label for="Name">Car Rent:</label>
                                <input type="number" name="su_price" class="form-control" value="{{$all->Price}}" required>
                            </div>
                            <div class="form-group">
                                <label for="Image">Upload Image:</label>
                                <input type="file" name="su_avatar" ><img src="{{$all->Image}}" alt="" height="50px" width="50px">
                            </div>
                            <center><button class="btn btn-lg btn-primary">Update</button></center>
                        </form><br><br>
                        <center><a href="{{url('/suvsection')}}">Back >></a></center>
                        @endforeach
